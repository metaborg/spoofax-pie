...
/**
 * Copied from org.eclipse.jdt.internal.ui.javaeditor.ToggleCommentAction.
 */
public class ToggleCommentHandler extends AbstractHandler {
    @Override public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if(!(part instanceof SpoofaxEditorBase)) {
            return null;
        }
        final SpoofaxEditorBase editor = (SpoofaxEditorBase)part;
        final IDocument document = editor.getDocument();
        final ISelection selection = editor.getSelectionProvider().getSelection();

        final SourceViewerConfiguration configuration = editor.getSourceViewerConfigurationReally();
        final ISourceViewer sourceViewer = editor.getSourceViewerReally();
        ...
        final ITextOperationTarget textOperationTarget = editor.getTextOperationTarget();
    ...
    private boolean isSelectionCommented(IDocument document, ISelection selection, String partitioning,
        ...
        try {
            ...
            //logger.error("Toggling comment failed", e); // TODO: log
        }
