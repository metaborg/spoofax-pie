...
class compile : Builder<compile.Input, PPath> {
  ...
  override fun BuildContext.build(input: compile.Input): PPath {
    ...
    input.includeDirs.map { dir -> require(dir, DirModifiedPathStamper(PPaths.patternPathMatcher("*.h"))) }.forEach { }
    ...
    mb.pipe.run.ceres.clang.extractCompileDeps(depFileText).map { dep -> require(dep, HashPathStamper()) }.forEach { }
