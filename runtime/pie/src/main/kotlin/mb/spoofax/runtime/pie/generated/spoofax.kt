...
class langSpecConfigForPath : Builder<langSpecConfigForPath.Input, LangSpecConfig?> {
  ...
  override fun BuildContext.build(input: langSpecConfigForPath.Input): LangSpecConfig? {
...
class spxCoreConfigForpath : Builder<spxCoreConfigForpath.Input, SpxCoreConfig?> {
  data class Input(val workspace: WorkspaceConfig, val path: PPath) : Tuple2<WorkspaceConfig, PPath> {
    constructor(tuple: Tuple2<WorkspaceConfig, PPath>) : this(tuple.component1(), tuple.component2())
  }

  override val id = "spxCoreConfigForpath"
  override fun BuildContext.build(input: spxCoreConfigForpath.Input): SpxCoreConfig? {
    val extension = input.path.extension()
    if (extension == null) return null
...
class createWorkspaceConfig : Builder<PPath, WorkspaceConfig?> {
  override val id = "createWorkspaceConfig"
  override fun BuildContext.build(input: PPath): WorkspaceConfig? {
    val cfgLang = SpxCoreConfig.create(PPathImpl(Paths.get("/Users/gohla/metaborg/repo/pipeline/cfg/langspec")) as PPath, false, list("cfg"))
...
class processProject : Builder<processProject.Input, ArrayList<Tuple4<PPath, ArrayList<Token>?, ArrayList<Msg>, Styling?>>> {
  ...
  override fun BuildContext.build(input: processProject.Input): ArrayList<Tuple4<PPath, ArrayList<Token>?, ArrayList<Msg>, Styling?>> {
    ...
    val langSpecResults = requireOutput(WalkContents::class.java, WalkContents.Input(input.project, PPaths.extensionsPathWalker(workspace.langSpecExtensions()))).map { file -> requireOutput(processFileWithLangSpecConfig::class.java, processFileWithLangSpecConfig.Input(file, input.project, workspace)) }.toCollection(ArrayList<Tuple4<PPath, ArrayList<Token>?, ArrayList<Msg>, Styling?>>())
    val spxCoreResults = requireOutput(WalkContents::class.java, WalkContents.Input(input.project, PPaths.extensionsPathWalker(workspace.spxCoreExtensions()))).map { file -> requireOutput(processFileWithSpxCore::class.java, processFileWithSpxCore.Input(file, input.project, workspace)) }.toCollection(ArrayList<Tuple4<PPath, ArrayList<Token>?, ArrayList<Msg>, Styling?>>())
...
class processString : Builder<processString.Input, processString.Output?> {
  ...
  private fun output(tuple: Tuple3<ArrayList<Token>?, ArrayList<Msg>, Styling?>?) = if (tuple == null) null else Output(tuple)
  ...
  override fun BuildContext.build(input: processString.Input): processString.Output? {
    ...
    if (langSpecConfig != null) {
      return output(requireOutput(processStringWithLangSpecConfig::class.java, processStringWithLangSpecConfig.Input(input.text, input.associatedProject, langSpecConfig!!, workspace)) as Tuple3<ArrayList<Token>?, ArrayList<Msg>, Styling?>?)
    }
    val spxCoreConfig = workspace.spxCoreConfigForExt(extension!!)
    if (spxCoreConfig != null) {
      return output(requireOutput(processStringWithSpxCore::class.java, processStringWithSpxCore.Input(input.text, input.associatedFile, spxCoreConfig!!)) as Tuple3<ArrayList<Token>?, ArrayList<Msg>, Styling?>?)
    }
    return null
  }
}
...
class processStringWithLangSpecConfig : Builder<processStringWithLangSpecConfig.Input, processStringWithLangSpecConfig.Output> {
  data class Input(val text: String, val associatedProject: PPath, val langSpec: LangSpecConfig, val workspace: WorkspaceConfig) : Tuple4<String, PPath, LangSpecConfig, WorkspaceConfig> {
    constructor(tuple: Tuple4<String, PPath, LangSpecConfig, WorkspaceConfig>) : this(tuple.component1(), tuple.component2(), tuple.component3(), tuple.component4())
  }
  ...
  override fun BuildContext.build(input: processStringWithLangSpecConfig.Input): processStringWithLangSpecConfig.Output {
    ...
    val styling: Styling? = if (tokenStream == null) null else requireOutput(style::class.java, style.Input(tokenStream!!, input.langSpec, input.workspace))
...
class style : Builder<style.Input, Styling?> {
  data class Input(val tokenStream: ArrayList<Token>, val langSpec: LangSpecConfig, val workspace: WorkspaceConfig) : Tuple3<ArrayList<Token>, LangSpecConfig, WorkspaceConfig> {
    constructor(tuple: Tuple3<ArrayList<Token>, LangSpecConfig, WorkspaceConfig>) : this(tuple.component1(), tuple.component2(), tuple.component3())
  }
  ...
  override fun BuildContext.build(input: style.Input): Styling? {
    ...
    return requireOutput(mb.spoofax.runtime.pie.builder.Style::class.java, mb.spoofax.runtime.pie.builder.Style.Input(input.tokenStream, syntaxStyler!!)) as Styling?
  }
}
...
class processStringWithSpxCore : Builder<processStringWithSpxCore.Input, processStringWithSpxCore.Output> {
  data class Input(val text: String, val associatedFile: PPath, val config: SpxCoreConfig) : Tuple3<String, PPath, SpxCoreConfig> {
  ...
  override fun BuildContext.build(input: processStringWithSpxCore.Input): processStringWithSpxCore.Output {
    ...
    val styling: Styling? = if (ast == null || tokens == null) null else requireOutput(mb.spoofax.runtime.pie.builder.core.CoreStyle::class.java, mb.spoofax.runtime.pie.builder.core.CoreStyle.Input(input.config, tokens!!, ast!!)) as Styling?
