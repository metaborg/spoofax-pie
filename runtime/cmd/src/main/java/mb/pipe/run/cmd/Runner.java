...
public class Runner {
    ...
    @Inject
    public Runner(PathSrv resourceService, BuildManagerFactory buildManagerFactory) {
    ...
    public int run(String[] args) throws Throwable {
        ...
        try (final BuildStore store = new LMDBBuildStore(new File("target/ceres"), 1024 * 1024 * 128, 8)) {
            clean(context, arguments, store, cache);
    ...
    private void build(Context context, Arguments arguments, BuildStore store, BuildCache cache) {
        ...
        final PPath resource = result.component1();
    ...
    private static void clean(Context context, Arguments arguments, BuildStore store, BuildCache cache) throws Throwable {
        if (!arguments.clean) {
            return;
        }
